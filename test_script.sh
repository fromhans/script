...
echo "nohup ${TARGET_DIRS}/bin/zookeeper-server-start.sh ${TARGET_DIRS}/config/zookeeper.properties 1>/dev/null 2>&1 &" >> /etc/rc.d/rc.local
echo "nohup ${TARGET_DIRS}/bin/kafka-server-start.sh ${TARGET_DIRS}/config/server.properties" >> /etc/rc.d/rc.local
